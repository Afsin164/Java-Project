...
public class BookTrains extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			// 🛡️ Validate date is today or future
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
			LocalDate journeyLocalDate = LocalDate.parse(journeyDate, formatter);
			LocalDate currentDate = LocalDate.now();

			if (journeyLocalDate.isBefore(currentDate)) {
				pw.println("<div class='tab'><p1 class='menu red'>Invalid Date! You cannot book for past dates.</p1></div>");
				return;
			}
			...
			if (train != null) {
				...
				if (seat > avail) {
					...
					if (ResponseCode.SUCCESS.toString().equalsIgnoreCase(responseCode)) {
						...
						bookingDetails.setDate(journeyDate);
